...
var FAKE_OUTLET = {
    setPowerOn: function (on) {
        console.log("Turning the LCD Light %s!...", on ? "on" : "off");
        if (on) {
            FAKE_OUTLET.powerOn = true;
            if (err) {
                return console.log(err);
            }
            console.log("...LCD Light is now on.");
            pyshell.send("{'M':'LL','V':1}\n");
        } else {
            FAKE_OUTLET.powerOn = false;
            if (err) {
                return console.log(err);
            }
            console.log("...LCD Light is now off.");
            pyshell.send("{'M':'LL','V':0}\n");
        }
    },
    identify: function () {
        console.log("Identify the LCD Light.");
    }
};
...
var outletUUID = uuid.generate('hap-nodejs:accessories:LCD Light');

// This is the Accessory that we'll return to HAP-NodeJS that represents our fake light.
var outlet = exports.accessory = new Accessory('LCD Light', outletUUID);

// Add properties for publishing (in case we're using Core.js and not BridgedCore.js)
outlet.username = "1A:2B:3F:4D:5D:FF";
...
outlet
    .addService(Service.Outlet, "LCD Light") // services exposed to the user should have "names" like "Fake Light" for us
